...
* Como <kbd>' apóstrofe</kbd> cuando la siguiente letra puede usarse tras el apóstrofe inglés (s,l,m,d,r,t,c), por ejemplo _McDonald's_
...
* Como <img src="https://raw.githubusercontent.com/alvaro-prieto/corne/main/extra/keys_v2/deleteword.svg" width="20"> tecla de borrado rápido de palabras completas (si se pulsa más de una vez)
...
* Posibilidad de alternar entre lenguajes: <kbd>inglés</kbd> / <kbd>español</kbd>. En español las teclas acentuadas se consiguen con una doble pulsación de la vocal en cuestión, por ejemplo: ``á = a + a``. A diferencia de los tap dances tradicionales, no hay ningún tipo de delay entre la pulsación y la aparición de la primera `a`. Lo mismo sucede con la ``ñ = n + n``, y también con con la `g`, siendo una doble pulsación equivalente a ``g¨``. Se puede alternarnar entre inglés y español presionando ``CFG + L`` para evitar este comportamiento.
