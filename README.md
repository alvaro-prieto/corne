...
* Posibilidad de alternar entre lenguajes: inglés / español. En español las teclas acentuadas se consiguen con una doble pulsación de la vocal en cuestión, por ejemplo: ``á = a + a``. A diferencia de los tap dances tradicionales, no hay ningún tipo de delay entre la pulsación y la aparición de la primera `a`. Lo mismo sucede con la ``ñ = n + n``, y también con con la `g`, siendo una doble pulsación equivalente a ``g¨``. Se puede alternarnar entre inglés y español presionando ``CFG + L`` para evitar este comportamiento.
